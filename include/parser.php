...
function strip_empty_bbcode($text)
{
	...
	// Remove empty tags
	while (($new_text = preg_replace('%\[(b|u|s|ins|del|em|i|h|colou?r|quote|img|url|email|list|topic|post|forum|user|youtube)(?:\=[^\]]*)?\]\s*\[/\1\]%', '', $text)) !== NULL)
	{
		if ($new_text != $text)
			$text = $new_text;
		else
...
function preparse_tags($text, &$errors, $is_signature = false)
{
	...
	$tags = array('quote', 'code', 'b', 'i', 'u', 's', 'ins', 'del', 'em', 'color', 'colour', 'url', 'email', 'img', 'list', '*', 'h', 'topic', 'post', 'forum', 'user','youtube');
